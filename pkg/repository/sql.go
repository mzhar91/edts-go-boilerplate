...
import (
	...
	"sg-edts.com/edts-go-boilerplate/pkg/types"
)
	...
	for i := 0; i < val.NumField(); i++ {
		...
		if column != "" {
			...
			case null.Bool:
				...
				if cse == "nullable" && !value.(null.Bool).Valid {
					value = nil
				} else {
			...
			case uuid.NullUUID:
				...
				break
			case types.NullUUID:
				if (value.(types.NullUUID).UUID.String() != defaultUUID && len(value.(types.NullUUID).UUID.String()) > 0) || cse == "nullable" {
					isEmpty = false
					
					if value.(types.NullUUID).UUID.String() != defaultUUID && len(value.(types.NullUUID).UUID.String()) > 0 {
						value = value.(types.NullUUID).UUID.String()
					} else {
						value = nil
					}
				}
